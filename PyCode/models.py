...
class MultiScaleDepNet(nn.Module):
    def __init__(self,
                 scales: tuple = (1,9),
                 num_samples: int = 768,
                 num_channels: int = 11,
                 ...
                 kernel_2: int = 16,
                 dropout: float = 0.25):
        super(MultiScaleDepNet, self).__init__()
        self.F1 = F1
        self.F2 = F2
        self.D = D
        self.num_samples = num_samples
        ...
        self.kernel_2 = kernel_2
        self.dropout = dropout
        self.scale_time = [pow(2, scale_time) for scale_time in range(scales[0],scales[1])]
        ...
        self.block1 = nn.ModuleList([
            ...
            ) for scale in self.scale_time
            ])

        self.block2 = Classifier(len(self.scale_time), num_classes)
        
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = torch.cat([self.block1[netindex](x).unsqueeze(2) for netindex in range(len(self.block1))], dim=2)
        x = self.block2(x)
        return x